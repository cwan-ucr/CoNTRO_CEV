...
class PPOWorker(Worker):
    ...
    def train_rollout(self, total_step):
        ...
        while rollout_amt < self.constants['episode']['rollout_length']:
            ...
            next_state, reward, done, real_action = self.env.step(action, self.ep_step, get_global_reward=False) #real_action is the real value after action excution
            self.ep_step += 1
            if done:
            ...
            if self.ep_step > self.constants['episode']['warmup_ep_steps']:
                # prediction['a'] = torch.tensor(real_action, device=self.device) # store the real actions
                ...
                if np.isnan(reward).any() or np.isinf(reward).any():
                    print(f"[Worker {self.env.agent_ID}] Invalid reward detected: {reward}")
        ...
        if torch.isnan(advantages).any() or torch.isinf(advantages).any():
            print("[Debug] Detected NaN/Inf in advantages before normalization")
            print("Mean:", advantages.mean().item(), "Std:", advantages.std().item())
            print("Rewards:", storage.r)
        eps = 1e-8
        ...
        if rollout_amt == 0:
            print("[Warning] No rollout data collected. Skipping training step.")
            return total_step
        ...
        mean_reward = torch.stack(storage.r).mean().item()
        min_reward = torch.stack(storage.r).min().item()
        max_reward = torch.stack(storage.r).max().item()

        print(f"[Worker {self.env.agent_ID}] Rollout reward stats → Mean: {mean_reward:.6f}, Min: {min_reward:.6f}, Max: {max_reward:.6f}")
